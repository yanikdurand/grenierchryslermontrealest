-- `type_vehicule` manquait dans la façade — le frontend ne peut pas
-- distinguer neuf d'occasion tant qu'il n'est pas exposé ici.
 ...
 SELECT v.id,
   ...
   FROM vehicule v
     ...
     LEFT JOIN LATERAL ( SELECT vente.id,
         ...
         LIMIT 1) ve ON true
     LEFT JOIN LATERAL ( SELECT l_1.description AS garantie_description,
            l_1.garantie_lieu,
            l_1.garantie_rdv,
            l_1.garantie_parti_le,
            l_1.garantie_retour_le
           FROM inspection_ligne l_1
             JOIN inspection i2 ON i2.id = l_1.inspection_id
          WHERE i2.vehicule_id = v.id AND l_1.sous_garantie
          ORDER BY (l_1.garantie_retour_le IS NULL) DESC, l_1.garantie_rdv DESC NULLS LAST
         LIMIT 1) g ON true;
